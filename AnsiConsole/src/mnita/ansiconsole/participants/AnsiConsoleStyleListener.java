...
public class AnsiConsoleStyleListener implements LineStyleListener, IPositionUpdater {
    ...
    static private void addRange(List<StyleRange> ranges, int start, int length,
    ...
    public void lineGetStyle(LineStyleEvent event) {
        ...
        if (documentEverScanned) {
            ...
            calculateDocumentAnsiPositions(document, event.lineOffset, event.lineText.length(), event.lineText);
        }
    ...
    private static void calculateDocumentAnsiPositions(IDocument eventDocument, int offset, int length, String text) {
        if (length != 0) { // This is the length of the text replaced. If not zero then this is not append, is replace.
            return;
        }
        if (offset + text.length() != eventDocument.getLength()) { // The appended text is at the end, so it is not inserted text
            return;
        }
        try {
